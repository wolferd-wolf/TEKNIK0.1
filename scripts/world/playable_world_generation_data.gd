...
# The shipping chunk cache reuses those lattice nodes across all columns; direct
# world queries use the same interpolation contract so mesh/gameplay heights stay
# identical at chunk boundaries and during mining/placement.
...
func stage3_macro_warp_offset(x: int, z: int) -> Vector2:
	...
	var tx := _stage3_smooth01(
		float(x - origin_x) * STAGE3_WARP_LATTICE_RECIPROCAL
	)
	var tz := _stage3_smooth01(
		float(z - origin_z) * STAGE3_WARP_LATTICE_RECIPROCAL
	)
...
func stage3_terrain_structure(x: int, z: int) -> float:
	...
	var north_east := stage3_sample_structure_node(
		origin_x + STAGE3_FIELD_LATTICE_SPACING,
		origin_z
	)
	var south_west := stage3_sample_structure_node(
		origin_x,
	...
	var north := lerpf(north_west, north_east, tx)
	var south := lerpf(south_west, south_east, tx)
