...
static func build(
	...
	# Stage 9 owns all generated tree eligibility, including Plains/Forest. First
	# suppress every legacy Stage 6 tree that could affect this chunk, then inject
	# only the terrain-filtered Stage 9 origins below. Existing water/lake blocked
	# columns remain part of the same suppression set.
	...
	# Apply terrain-aware ground expression to the one-column halo that can affect
	# rendered chunk faces/AO. The ecology byte itself is untouched.
	for cache_z in range(1, cache_width - 1):
		for cache_x in range(1, cache_width - 1):
			var index: int = cache_z * cache_width + cache_x
			var world_x: int = origin.x + cache_x - cache_padding
			...
			for y in range(maxi(0, surface - 1), surface + 1):
				...
				if overrides.has(key):
					continue
				var desired: int = sampler.stage9_surface_block(
					cell, surface, biome, modifier, slope
				)
				var baseline: int = BASE_MESHER._terrain_block(y, surface, sea_level, biome)
				if desired != baseline:
					mesher_overrides[key] = desired

	# Collect exact Stage 9 tree origins. Tree silhouettes remain Stage 8's six
	# ecology silhouettes; only terrain eligibility/density changes here.
	...
	for cache_z in range(1, cache_width - 1):
		for cache_x in range(1, cache_width - 1):
			...
			var slope: float = _cached_slope(cache_x, cache_z, heights, cache_width)
			if not sampler.stage9_tree_candidate_for_biome(
	...
	# Trunks first so overlapping foliage never hides a mineable log.
