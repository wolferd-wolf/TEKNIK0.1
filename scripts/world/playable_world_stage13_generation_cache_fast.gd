...
static func build(coord: Vector2i, sampler) -> Dictionary:
	...
	for cz in range(width):
		...
		for cx in range(width):
			...
			# Stage 8 ecology and Stage 9 modifier are written directly here instead
			# of generating Stage 7's transitional four-biome result and replacing it
			# in a second 256-column pass.
	...
	# Stage 6 basin placement and shaping. Wet cores override ecology to Plains and
	# terrain modifier to none, exactly matching the Stage 9 post-pass semantics.
