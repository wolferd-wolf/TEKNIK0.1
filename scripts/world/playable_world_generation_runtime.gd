...
func _build_column_caches(coord: Vector2i) -> Dictionary:
	return SHIPPING_STAGE5_CACHE.build(coord, data)
...
static func _stage3_worker_build_chunk(
	...
	var sampler = SHIPPING_STAGE5_DATA.new()
	var cache_started_usec := Time.get_ticks_usec()
	var caches: Dictionary = SHIPPING_STAGE5_CACHE.build(coord, sampler)
	...
	var mesh_data: Dictionary = SHIPPING_STAGE5_MESHER.build(
		coord,
		heights,
		overrides_snapshot,
		12,
		mesh_height,
		SHIPPING_STAGE5_DATA.SEA_LEVEL,
		biomes
	)
	var result := {
		"coord": coord,
