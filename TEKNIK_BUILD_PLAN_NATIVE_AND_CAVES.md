...
## Decisions
- Deferred item resolved (pixel textures): NOT merging `feature/procedural-pixel-textures`. Ore readability is handled by distinct flat hues per ore/ingot id, byte-exact in both meshers; texture merging stays deferred.
- Steps 14-16 ids: coal ore 10, iron ore 11, copper ore 12, furnace 13, iron ingot 14, copper ingot 15, coal 16, glass 17, charcoal 18.
- Ore policy lives in GDScript (`ore_field_reference.gd`) on top of the parity-proven hash primitive; no Rust port (Rust owns field math, GDScript owns placement policy).
- Smelting semantics: 1 input + 1 fuel -> 1 output, button-batched with sequential timed processing in the UI; no persistent furnace state (inventory-only), so rollback and determinism are trivially testable.
- Progression: iron-first metal path ADDED alongside the original wood path rather than replacing it, so the early game never dead-ends before the first furnace exists.
