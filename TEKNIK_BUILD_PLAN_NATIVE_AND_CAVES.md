...
1. Commit `native/carpathian/SConstruct` + `build_profile.json` to the repo (stop generating them inline in CI). Gitignore the `godot-cpp/` clone and `bin/` outputs. *Gate: existing native equivalence workflow passes unchanged.*
2. Commit a real `*.gdextension` file plus a GDScript `NativeLoader` singleton exposing `is_native_available()` with graceful fallback detection. *Gate: new headless gate proves fallback works with the library absent AND native works with the library present.*
