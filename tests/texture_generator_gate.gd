...
func _init() -> void:
	...
	var same := TextureGenerator.generate_face("dirt", "side", SEED).get_image().get_data()
	var different := TextureGenerator.generate_face("dirt", "side", SEED + 1).get_image().get_data()
	...
	var ore := TextureGenerator.generate_face("iron_ore", "side", SEED).get_image(); var ore_pixels := 0
	for y in range(32):
		for x in range(32):
