...
func _init() -> void:
	...
	for block_id in blocks:
		var a: Image = TextureGenerator.generate(block_id, SEED).get_image()
		var b: Image = TextureGenerator.generate(block_id, SEED).get_image()
	...
	var same_seed := TextureGenerator.generate("dirt", SEED).get_image().get_data()
	var other_seed := TextureGenerator.generate("dirt", SEED + 1).get_image().get_data()
	if same_seed == other_seed: failures.append("SEED_HAS_NO_EFFECT dirt")
	var ore := TextureGenerator.generate("iron_ore", SEED).get_image()
	var ore_mask := {}; var ore_pixels := 0
	for y in range(32):
		for x in range(32):
			var c := ore.get_pixel(x, y)
			if c.r > 0.58 and c.g < 0.50: ore_mask[Vector2i(x,y)] = true; ore_pixels += 1
...
func _largest_ore_cluster(mask: Dictionary) -> int:
	var visited := {}; var largest := 0
	for start in mask:
		if visited.has(start): continue
		var queue: Array[Vector2i] = [start]; visited[start] = true; var size := 0; var head := 0
		while head < queue.size():
			...
			for oy in range(-1,2):
				for ox in range(-1,2):
					...
					if mask.has(next) and not visited.has(next): visited[next] = true; queue.append(next)
