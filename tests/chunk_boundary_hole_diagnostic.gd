...
func _face_key(center: Vector3, normal: Vector3) -> String:
	return "%.3f,%.3f,%.3f|%.0f,%.0f,%.0f" % [center.x, center.y, center.z, normal.x, normal.y, normal.z]
...
func _mesh_face_lookup(coord: Vector2i, mesh_data: Dictionary) -> Dictionary:
	...
	for face in range(face_count):
		...
		lookup[_face_key(center, normal)] = true
	return lookup
...
func _solid(block: int) -> bool:
	return block != data_ref.BLOCK_AIR and block != data_ref.BLOCK_WATER

func _check_boundary_faces(a: Vector2i, b: Vector2i, data, label: String) -> void:
	var ma := _build_mesh(a, data)
	var mb := _build_mesh(b, data)
	var la := _mesh_face_lookup(a, ma)
	var lb := _mesh_face_lookup(b, mb)
	var dx := b.x - a.x
	var dz := b.y - a.y
	if absi(dx) + absi(dz) != 1:
		_fail("non-cardinal chunk pair %s %s" % [a,b])
		return
	if dx != 0:
		...
		for z in range(z0, z0 + CHUNK_SIZE):
			for y in range(data.OVERHAUL_WORLD_HEIGHT):
				var left := data.get_block(Vector3i(boundary_x - 1, y, z))
				var right := data.get_block(Vector3i(boundary_x, y, z))
				if _solid(left) and not _solid(right):
					expected_boundary_faces += 1
					var key := _face_key(Vector3(boundary_x, y + 0.5, z + 0.5), Vector3.RIGHT)
					if not left_lookup.has(key):
						missing_boundary_faces += 1
						if details_printed < 30:
							print("BOUNDARY_FACE_MISSING label=%s owner=%s world=(%d,%d,%d) normal=RIGHT left=%d right=%d" % [label,left_chunk,boundary_x-1,y,z,left,right])
							details_printed += 1
				elif _solid(right) and not _solid(left):
					expected_boundary_faces += 1
					var key := _face_key(Vector3(boundary_x, y + 0.5, z + 0.5), Vector3.LEFT)
					if not right_lookup.has(key):
						missing_boundary_faces += 1
						if details_printed < 30:
							print("BOUNDARY_FACE_MISSING label=%s owner=%s world=(%d,%d,%d) normal=LEFT left=%d right=%d" % [label,right_chunk,boundary_x,y,z,left,right])
							details_printed += 1
	else:
		...
		for x in range(x0, x0 + CHUNK_SIZE):
			for y in range(data.OVERHAUL_WORLD_HEIGHT):
				var north := data.get_block(Vector3i(x, y, boundary_z - 1))
				var south := data.get_block(Vector3i(x, y, boundary_z))
				if _solid(north) and not _solid(south):
					expected_boundary_faces += 1
					var key := _face_key(Vector3(x + 0.5, y + 0.5, boundary_z), Vector3.BACK)
					if not north_lookup.has(key):
						missing_boundary_faces += 1
						if details_printed < 30:
							print("BOUNDARY_FACE_MISSING label=%s owner=%s world=(%d,%d,%d) normal=BACK north=%d south=%d" % [label,north_chunk,x,y,boundary_z-1,north,south])
							details_printed += 1
				elif _solid(south) and not _solid(north):
					expected_boundary_faces += 1
					var key := _face_key(Vector3(x + 0.5, y + 0.5, boundary_z), Vector3.FORWARD)
					if not south_lookup.has(key):
						missing_boundary_faces += 1
						if details_printed < 30:
							print("BOUNDARY_FACE_MISSING label=%s owner=%s world=(%d,%d,%d) normal=FORWARD north=%d south=%d" % [label,south_chunk,x,y,boundary_z,north,south])
							details_printed += 1
...
func _init() -> void:
	...
	for probe in PROBES:
		...
		if local_x == CHUNK_SIZE - 1:
			...
			_check_shared_cache(chunk, east, data_ref, String(probe["label"]) + "_east")
			_check_boundary_faces(chunk, east, data_ref, String(probe["label"]) + "_east")
		if local_z == CHUNK_SIZE - 1:
			var south := chunk + Vector2i(0, 1)
			_check_shared_cache(chunk, south, data_ref, String(probe["label"]) + "_south")
			_check_boundary_faces(chunk, south, data_ref, String(probe["label"]) + "_south")
