...
func _run_gate() -> void:
	...
	for iteration in range(ITERATIONS):
		...
		chunk.rebuild_mesh(Callable(self, "_air_lookup"))
		if chunk.mesh_instance.mesh == null:
			_fail("Empty remesh returned null at iteration %d" % iteration)
			break
		if chunk.mesh_instance.mesh.get_surface_count() != 0:
			_fail("Empty remesh retained surfaces at iteration %d" % iteration)
			break
		if chunk.collision_shape.shape != null:
			_fail("Empty remesh retained collision at iteration %d" % iteration)
			break
