...
func _init() -> void:
	...
	var adapter_script := load("res://scripts/player/mobile_camera_sensitivity.gd")
	var adapter := adapter_script.new()
	adapter.name = "MobileCameraSensitivity"
	adapter.player_path = NodePath("../Player")
	adapter.force_enabled_for_tests = true
	adapter.radians_per_drag_pixel = 0.0035
	host.add_child(adapter)
	await process_frame

	_expect(adapter.is_mobile_look_enabled(), "adapter must enable in forced test mode", failures)
	...
	adapter._input(left_touch)
	_expect(adapter.get_active_look_touch_index() == -1, "left-half touch must not claim camera look", failures)
	...
	adapter._input(bottom_touch)
	_expect(adapter.get_active_look_touch_index() == -1, "bottom action area must remain reserved", failures)

	var look_touch := InputEventScreenTouch.new()
	look_touch.index = 3
	look_touch.position = Vector2(1000, 240)
	look_touch.pressed = true
	adapter._input(look_touch)
	_expect(adapter.get_active_look_touch_index() == 3, "right-side look touch must be captured", failures)
	...
	adapter._input(unrelated_drag)
	_expect(player.applied_deltas.size() == 1, "unclaimed touch must not rotate camera", failures)

	look_touch.pressed = false
	adapter._input(look_touch)
	_expect(adapter.get_active_look_touch_index() == -1, "look touch release must clear capture", failures)
