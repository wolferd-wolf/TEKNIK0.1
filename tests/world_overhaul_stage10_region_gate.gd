...
func _validate_contract() -> Dictionary:
	...
	if not cache_source.contains("STAGE7_CACHE.build"):
		_fail("Stage 10 no longer reuses the accepted fused geography cache")
	if cache_source.contains("STAGE9_CACHE.build"):
		_fail("Stage 10 stacks another post-pass on Stage 9 instead of replacing it")
...
func _validate_synthetic(data) -> Dictionary:
	var boundary_climate := Vector2(0.0, 0.18)
	var boundary_biome: int = data.stage8_classify_climate(boundary_climate, DATA.WATER_NONE)
	var boundary_code: int = data.stage10_transition_code_for_climate(
		boundary_climate, DATA.WATER_NONE
	)
	var boundary_partner: int = data.stage10_transition_partner(boundary_code)
	var boundary_level: int = data.stage10_transition_level(boundary_code)
	if boundary_biome != DATA.BIOME_PLAINS:
		_fail("Stage 10 changed the exact Plains/Forest boundary tie winner")
	if boundary_partner != DATA.BIOME_FOREST:
		_fail("Stage 10 does not expose Forest as the Plains boundary partner")
	if boundary_level < DATA.STAGE10_TRANSITION_LEVELS - 1:
		_fail("Stage 10 climate boundary does not reach full transition strength")
	if data.stage10_transition_code_for_climate(boundary_climate, DATA.WATER_RIVER) != 0:
	...
	return {
		"boundary_biome": boundary_biome,
		"boundary_partner": boundary_partner,
...
func _validate_stage9_equivalence(data, runtime) -> Dictionary:
	...
	for coord: Vector2i in coords:
		...
		var transitions: PackedByteArray = stage10.get(
			"stage10_transition_codes", PackedByteArray()
		)
		if transitions.size() != WIDTH * WIDTH:
			_fail("Stage 10 transition cache has the wrong padded size in chunk %s" % coord)
			continue
		...
		for local_z in range(CHUNK_SIZE):
			for local_x in range(CHUNK_SIZE):
				...
				if int(transitions[index]) != expected:
					_fail("Stage 10 cache/public transition mismatch at (%d,%d)" % [world_x, world_z])
...
func _audit_world(data) -> Dictionary:
	...
	for chunk_z: int in chunk_axis:
		for chunk_x: int in chunk_axis:
			...
			var transitions: PackedByteArray = cache.get("stage10_transition_codes", PackedByteArray())
			if transitions.size() != WIDTH * WIDTH:
				_fail("Stage 10 broad audit encountered malformed transition cache")
				continue
			var origin_x: int = coord.x * CHUNK_SIZE
			var origin_z: int = coord.y * CHUNK_SIZE
...
func _audit_regions(data) -> Dictionary:
	...
	if not transition_runs.is_empty():
		var middle: int = transition_runs.size() / 2
...
func _validate_seams(runtime) -> Dictionary:
	...
	for pair in pairs:
		...
		for key in ["biomes", "heights", "stage9_terrain_modifiers", "stage10_transition_codes"]:
			var values_a = cache_a.get(key)
			var values_b = cache_b.get(key)
...
func _validate_determinism(data) -> Dictionary:
	...
	for coord: Vector2i in coords:
		...
		for key in ["heights", "biomes", "stage7_water_types", "stage9_terrain_modifiers", "stage10_transition_codes"]:
	...
	for _repeat in range(REPEATS):
		for coord: Vector2i in coords:
			...
			if not transitions.is_empty():
				checksum += int(transitions[0]) + int(transitions[transitions.size() - 1])
