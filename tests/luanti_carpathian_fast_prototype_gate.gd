...
func _init() -> void:
	...
	for seed in seeds:
		...
		print("CARPATHIAN_FAST_SEED=%d height_min=%d height_max=%d height_mean=%.4f height_p05=%d height_p50=%d height_p95=%d slope_le_1_pct=%.4f slope_le_2_pct=%.4f largest_low_relief_cells=%d largest_low_relief_equiv_width_blocks=%.4f elapsed_ms=%.3f" % [
			...
			result["p05"], result["p50"], result["p95"],
	...
	if not all_ok:
		push_error("Carpathian fast prototype determinism failed")
		quit(1)
		return
	print("LUANTI_CARPATHIAN_FAST_PROTOTYPE_COMPLETE")
	quit(0)
...
func _audit_seed(seed: int, out_dir: String) -> Dictionary:
	...
	for z in range(GRID):
		for x in range(GRID):
			var wx := (x - half) * STEP
			var wz := (z - half) * STEP
			var h := gen.surface_height_raw(wx, wz)
			var i := z * GRID + x
			heights[i] = h
			min_h = mini(min_h, h)
			max_h = maxi(max_h, h)
	...
	for z in range(GRID):
		for x in range(GRID):
			...
			var slope := 0
			if x > 0:
				slope = maxi(slope, absi(heights[i] - heights[i - 1]))
			if x + 1 < GRID:
				slope = maxi(slope, absi(heights[i] - heights[i + 1]))
			if z > 0:
				slope = maxi(slope, absi(heights[i] - heights[i - GRID]))
			if z + 1 < GRID:
				slope = maxi(slope, absi(heights[i] - heights[i + GRID]))
			if slope <= 1:
				slope1_count += 1
	...
	for start in range(count):
		if low[start] == 0 or seen[start] != 0:
			continue
		queue.clear()
		queue.append(start)
		seen[start] = 1
		...
		while head < queue.size():
			...
			if x + 1 < GRID:
				_try_add(i + 1, low, seen, queue)
			if z > 0:
				_try_add(i - GRID, low, seen, queue)
			if z + 1 < GRID:
				_try_add(i + GRID, low, seen, queue)
	...
	for z in range(GRID):
		for x in range(GRID):
			var h := heights[z * GRID + x]
			var value := clampf(float(h - min_h) / denom, 0.0, 1.0)
...
func _benchmark_chunk() -> Dictionary:
	...
	for rep in range(12):
		...
		for z in range(CHUNK_SIZE):
			for x in range(CHUNK_SIZE):
				gen.surface_height_raw(x0 + x, z0 + z)
		var ended := Time.get_ticks_usec()
