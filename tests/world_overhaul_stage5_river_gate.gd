...
func _init() -> void:
	...
	var river_chunk: Vector2i = Vector2i(
...
func _contract(data) -> Dictionary:
	...
	if DATA.STAGE5_RIVER_LATTICE_SPACING < 64:
		_fail("River lattice is too fine")
	if DATA.STAGE5_MAX_VALLEY_CARVE > 32:
		_fail("River valley carve safety cap is too large")
	var source: String = FileAccess.get_file_as_string("res://scripts/world/playable_world_stage5_generation_data.gd")
	var cache_source: String = FileAccess.get_file_as_string("res://scripts/world/playable_world_stage5_cache_fast.gd")
	...
	if not cache_source.contains("river_nodes") or not cache_source.contains("river_signal"):
		_fail("Shipping cache does not cache the river field")
...
func _synthetic(data, stage4) -> Dictionary:
	var fields: Vector4 = Vector4(0.0, 0.82, 0.0, 0.0)
	var provisional: int = int(data.build_provisional_terrain(fields))
	var stage4_height: int = int(stage4.finalize_height(stage4.apply_water_topology(fields, provisional, 0, 0)))
	var scale: float = float(data.stage5_river_width_scale(fields.x))
	var center: int = int(data.stage5_shape_height_from_signal(fields.x, stage4_height, 0.0))
	var shoulder_value: float = DATA.STAGE5_VALLEY_INNER * scale * 1.35
	var outside_value: float = DATA.STAGE5_VALLEY_OUTER * scale * 1.25
	var shoulder: int = int(data.stage5_shape_height_from_signal(fields.x, stage4_height, shoulder_value))
	var outside: int = int(data.stage5_shape_height_from_signal(fields.x, stage4_height, outside_value))
	var carve: int = stage4_height - center
...
func _field_stats(data) -> Dictionary:
	var samples: int = 0
	var channel_samples: int = 0
	var valley_samples: int = 0
	var max_neighbor_delta: float = 0.0
	var max_width_delta: float = 0.0
	for z: int in range(-384, 385, 8):
		for x: int in range(-384, 385, 8):
			var value: float = float(data.stage5_river_signal(x, z))
			var east: float = float(data.stage5_river_signal(x + 1, z))
			var south: float = float(data.stage5_river_signal(x, z + 1))
...
func _world_audit(data, stage4) -> Dictionary:
	var river_grid: PackedByteArray = PackedByteArray()
	var ocean_grid: PackedByteArray = PackedByteArray()
	river_grid.resize(AUDIT_WIDTH * AUDIT_WIDTH)
	ocean_grid.resize(AUDIT_WIDTH * AUDIT_WIDTH)
	var sampled: int = 0
	var rivers: int = 0
	var oceans: int = 0
	var mountain_rivers: int = 0
	var mountain_carve_sum: int = 0
	var max_carve: int = 0
	var ocean_joins: int = 0
	var surface_min: int = 999999
	var surface_max: int = -999999
	var chosen_chunk: Vector2i = Vector2i.ZERO
	var found_chunk: bool = false
	var gz: int = 0
	for z: int in range(AUDIT_START, AUDIT_FINISH + 1, AUDIT_STEP):
		...
		for x: int in range(AUDIT_START, AUDIT_FINISH + 1, AUDIT_STEP):
			...
			var provisional: int = int(data.build_provisional_terrain(fields))
			var stage4_height: int = int(stage4.finalize_height(stage4.apply_water_topology(fields, provisional, x, z)))
			var is_ocean: bool = int(stage4.water_type_from_fields(fields, stage4_height)) == DATA.WATER_OCEAN
			var value: float = float(data.stage5_river_signal(x, z))
			var strengths: Vector2 = data.stage5_river_strengths_from_signal(fields.x, value)
			var stage5_height: int = int(data.finalize_height(data.stage5_shape_height_from_signal(fields.x, stage4_height, value)))
			var is_river: bool = (not is_ocean) and strengths.x >= DATA.STAGE5_CHANNEL_WATER_CUTOFF
			var grid_index: int = gz * AUDIT_WIDTH + gx
			if is_ocean:
				...
				rivers += 1
				var surface_y: int = stage5_height + 1
				...
				if not found_chunk and fields.x >= DATA.STAGE4_COAST_INLAND_END and surface_y > DATA.SEA_LEVEL + 1:
					...
					found_chunk = true
				var carve: int = stage4_height - stage5_height
			...
			gx += 1
		gz += 1
	var components: Dictionary = _component_stats(river_grid, ocean_grid)
...
func _component_stats(rivers: PackedByteArray, oceans: PackedByteArray) -> Dictionary:
	...
	var long_components: int = 0
	var short_fragments: int = 0
	var max_span: int = 0
	for start: int in range(rivers.size()):
		...
		visited[start] = 1
		var cursor: int = 0
		var min_x: int = AUDIT_WIDTH
		var max_x: int = -1
		var min_z: int = AUDIT_WIDTH
		var max_z: int = -1
		...
		while cursor < queue.size():
			...
			cursor += 1
			var gx: int = cell % AUDIT_WIDTH
			var gz: int = int(cell / AUDIT_WIDTH)
			min_x = mini(min_x, gx)
			max_x = maxi(max_x, gx)
			min_z = mini(min_z, gz)
			max_z = maxi(max_z, gz)
			...
			for offset: Vector2i in [Vector2i.LEFT, Vector2i.RIGHT, Vector2i.UP, Vector2i.DOWN]:
				...
				if nx < 0 or nz < 0 or nx >= AUDIT_WIDTH or nz >= AUDIT_WIDTH:
					continue
				var neighbor: int = nz * AUDIT_WIDTH + nx
				if oceans[neighbor] != 0:
					touches_ocean = true
				if rivers[neighbor] != 0 and visited[neighbor] == 0:
					visited[neighbor] = 1
					queue.append(neighbor)
		var span: int = maxi(max_x - min_x + 1, max_z - min_z + 1) * AUDIT_STEP
...
func _runtime_equivalence(runtime, data, river_chunk: Vector2i) -> Dictionary:
	...
	for coord: Vector2i in coords:
		...
		var values: PackedFloat32Array = first.get("river_signal", PackedFloat32Array())
		if values.size() != CACHE_WIDTH * CACHE_WIDTH:
			_fail("Stage 5 cache missing padded river values")
			continue
		...
		for lz: int in range(-PADDING, CHUNK_SIZE + PADDING):
			for lx: int in range(-PADDING, CHUNK_SIZE + PADDING):
				...
				if absf(values[index] - float(data.stage5_river_signal(wx, wz))) > 0.00002:
					_fail("Stage 5 cache/public river mismatch at (%d,%d)" % [wx, wz])
	...
	return {"compared_columns": compared, "river_columns": river_columns}
...
func _renderer(data, river_chunk: Vector2i) -> Dictionary:
	...
	for lz: int in range(CHUNK_SIZE):
		for lx: int in range(CHUNK_SIZE):
			var wx: int = river_chunk.x * CHUNK_SIZE + lx
			var wz: int = river_chunk.y * CHUNK_SIZE + lz
...
func _benchmark(runtime) -> Dictionary:
	...
	for value: int in times:
		total += value
	var p95_index: int = clampi(ceili(float(times.size()) * 0.95) - 1, 0, times.size() - 1)
