...
func _init() -> void:
	...
	var terrain_report := _measure_terrain_shape(data)
	var benchmark_report := _benchmark_real_chunk_generation_and_meshing(data)
...
func _validate_height_contract(data) -> void:
	...
	var synthetic_peak := data.terrain_height_from_samples(Vector4(1.0, 1.0, -1.0, -1.0))
...
func _measure_terrain_shape(data) -> Dictionary:
	var width := int((MAP_HALF_SPAN * 2) / MAP_SAMPLE_STEP) + 1
	var heights := PackedInt32Array()
	var legacy_heights := PackedInt32Array()
	heights.resize(width * width)
	legacy_heights.resize(width * width)
	var minimum := WORLD_DATA.WORLD_HEIGHT
	var maximum := 0
	var legacy_maximum := 0
	for row in range(width):
		var z := MAP_CENTER.y - MAP_HALF_SPAN + row * MAP_SAMPLE_STEP
		for column in range(width):
			var x := MAP_CENTER.x - MAP_HALF_SPAN + column * MAP_SAMPLE_STEP
			var samples: Vector4 = data.sample_column_noise(x, z)
			var height: int = data.terrain_height_from_samples(samples)
			var legacy_height := _legacy_40_height(samples)
	...
	sorted.sort()
	var median := sorted[int(sorted.size() * 0.50)]
	...
	for row in range(1, width - 1):
		for column in range(1, width - 1):
			var index := row * width + column
			var center := int(heights[index])
			var north := int(heights[index - width])
			var south := int(heights[index + width])
			var west := int(heights[index - 1])
			var east := int(heights[index + 1])
			...
			if center >= north + 2 and center >= south + 2 and center >= west + 2 and center >= east + 2:
				ridge_points += 1
	var shaped_edge_ratio := float(shaped_edges) / float(maxi(compared_edges, 1))
...
func _legacy_40_height(samples: Vector4) -> int:
	var base_height := 10.0 + samples.x * 6.4 + samples.y * 3.0
	var cold_t := clampf(
	...
	var land_factor := clampf((base_height - 6.0) / 3.0, 0.0, 1.0)
	land_factor = land_factor * land_factor * (3.0 - 2.0 * land_factor)
	var peak_strength := clampf((samples.y + 1.0) * 0.5, 0.0, 1.0)
	peak_strength *= peak_strength
	var mountain_rise := rocky_weight * land_factor * (4.0 + peak_strength * 11.0)
...
func _benchmark_real_chunk_generation_and_meshing(data) -> Dictionary:
	...
	for repetition in range(REPEATS):
		for index in range(coords.size()):
			var coord := coords[(index + repetition) % coords.size()]
			var start := Time.get_ticks_usec()
			var consumed := _consume_real_chunk(runtime, data, coord)
			var elapsed := maxi(1, Time.get_ticks_usec() - start)
...
func _consume_real_chunk(runtime, data, coord: Vector2i) -> Dictionary:
	...
	return {
		...
		"vertices": (mesh.get("vertices", PackedVector3Array()) as PackedVector3Array).size(),
		"height_checksum": height_checksum,
	}
