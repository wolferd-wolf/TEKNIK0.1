...
func _init() -> void:
	...
	if int(benchmark["p95_usec"]) >= P95_LIMIT_USEC:
		_fail(
			"Stage 7 generation exceeded the 1.0 ms p95 gate: %d usec"
...
func _validate_contract(_data) -> Dictionary:
	...
	var data_source: String = FileAccess.get_file_as_string(
		"res://scripts/world/playable_world_stage7_biome_data.gd"
	)
	var cache_source: String = FileAccess.get_file_as_string(
		"res://scripts/world/playable_world_stage7_cache_fast.gd"
	)
	...
	if not cache_source.contains("stage7_water_types"):
		_fail("Stage 7 cache does not expose physical water ownership")
	if cache_source.contains("STAGE5_CACHE.build") or cache_source.contains("STAGE6_CACHE.build"):
		_fail("Stage 7 hot path is layered over an older cache instead of fused")
	...
	if DATA.STAGE7_ROCKY_ELEVATION_MIN <= DATA.STAGE2_SAFE_TERRAIN_TOP:
		_fail("Stage 7 Rocky still has a flat-elevation eligibility shortcut")
	return {
		...
		"prototype_count": 4,
		"rocky_mountain_strength_min": DATA.STAGE7_ROCKY_MOUNTAIN_STRENGTH_MIN,
	}


func _validate_synthetic_classifier(data) -> Dictionary:
	var plains: int = data.stage7_classify_with_context(
		DATA.STAGE7_PLAINS_TARGET, 0.0, 0.0, 18, DATA.WATER_NONE, 0.0
	)
	var forest: int = data.stage7_classify_with_context(
		DATA.STAGE7_FOREST_TARGET, 0.0, 0.0, 18, DATA.WATER_NONE, 0.0
	)
	var desert: int = data.stage7_classify_with_context(
		DATA.STAGE7_DESERT_TARGET, 0.0, 0.0, 18, DATA.WATER_NONE, 0.0
	)
	var rocky: int = data.stage7_classify_with_context(
		DATA.STAGE7_ROCKY_TARGET, 1.0, 6.0, 18, DATA.WATER_NONE, 0.0
	)
	...
	var flat_rocky: int = data.stage7_classify_with_context(
		DATA.STAGE7_ROCKY_TARGET, 0.0, 0.0, 80, DATA.WATER_NONE, 0.0
	)
	if flat_rocky == DATA.BIOME_ROCKY:
		_fail("Rocky still paints flat terrain merely because it is elevated")

	var wet_forest: int = data.stage7_classify_with_context(
		DATA.STAGE7_FOREST_TARGET, 0.0, 0.0, 5, DATA.WATER_OCEAN, 1.0
	)
	if wet_forest != DATA.BIOME_PLAINS:
		_fail("Physical water was allowed to classify as Forest")

	var coast_rocky: int = data.stage7_classify_with_context(
		DATA.STAGE7_ROCKY_TARGET, 1.0, 5.0, 50, DATA.WATER_NONE, 0.9
	)
	if coast_rocky == DATA.BIOME_ROCKY:
		_fail("Rocky ignores coastal eligibility")

	var fallback: int = data.stage7_classify_with_context(
		Vector2(99.0, -99.0), 0.0, 0.0, 18, DATA.WATER_NONE, 0.0
	)
...
func _audit_world(data) -> Dictionary:
	# The climate fields are intentionally slow. Sample a broad fixed geography
	# rather than weakening mountain eligibility merely to hit every ecology near
	# spawn. Coordinates are chunk coordinates, so this covers ~2300 blocks on
	# each axis while keeping the audit deterministic and cheap enough for CI.
	var chunk_axis: Array[int] = [-96, -64, -32, 0, 32, 64, 96]
	var counts := PackedInt32Array()
	counts.resize(DATA.BIOME_COUNT)
	var water_columns := 0
	var rocky_columns := 0
	var context_columns := 0
	var context_samples := 0
	...
	for chunk_z: int in chunk_axis:
		for chunk_x: int in chunk_axis:
			...
			var water_types: PackedByteArray = cache.get(
				"stage7_water_types", PackedByteArray()
			)
			var heights: PackedInt32Array = cache.get("heights", PackedInt32Array())
			var fields: PackedFloat32Array = cache.get(
			...
			if (
				...
				or fields.size() != WIDTH * WIDTH * FIELD_STRIDE
			):
				_fail("Stage 7 fused cache arrays have the wrong padded size")
				continue
			...
			for local_z in range(PADDING, PADDING + CHUNK_SIZE):
				for local_x in range(PADDING, PADDING + CHUNK_SIZE):
					...
					var mountain_strength: float = data.stage7_mountain_strength(structure)
					var coast: float = data.stage7_coast_proximity_from_continentalness(
						continentalness
					)
					...
					if biome == DATA.BIOME_ROCKY:
						rocky_columns += 1
						if not data.stage7_rocky_eligible(
							mountain_strength,
							int(heights[index]),
							coast
						):
							_fail("Rocky appeared outside mountain/coast eligibility")

			# Slope is part of the Stage 7 context contract but intentionally not in
			# the chunk hot path yet. Exercise the on-demand API once per audited chunk.
			var sample_x: int = origin_x + int(CHUNK_SIZE / 2)
			var sample_z: int = origin_z + int(CHUNK_SIZE / 2)
			var context: Dictionary = data.stage7_context_at(sample_x, sample_z)
			for key: String in [
				"height",
				"mountain_strength",
				"slope",
				...
				if not context.has(key):
					_fail("Stage 7 context is missing %s" % key)
			maximum_slope = maxf(maximum_slope, float(context.get("slope", 0.0)))
			context_samples += 1

	for biome_id: int in [DATA.BIOME_PLAINS, DATA.BIOME_FOREST, DATA.BIOME_DESERT]:
		if counts[biome_id] <= 0:
			_fail("Stage 7 broad world audit did not reach %s" % data.biome_name(biome_id))
	if rocky_columns <= 0:
		_fail("Stage 7 broad world audit did not reach an eligible Rocky ecology")
	if water_columns <= 0:
		_fail("Stage 7 broad world audit did not exercise water eligibility")
	if maximum_slope <= 0.0:
		_fail("Stage 7 slope context never observed terrain relief")

	var count_report: Array[int] = []
	for biome_id in range(DATA.BIOME_COUNT):
	...
	return {
		...
		"maximum_context_slope": maximum_slope,
	}
	...
	for coord: Vector2i in coords:
		...
		for local_z in range(CHUNK_SIZE):
			for local_x in range(CHUNK_SIZE):
				var index: int = (
					(local_z + PADDING) * WIDTH + local_x + PADDING
				)
				...
				if int(biomes7[index]) != expected:
					_fail(
						"Stage 7 cache/public biome mismatch at (%d,%d)"
						% [world_x, world_z]
					)
				compared_biomes += 1
				compared_heights += 1
	return {
		"height_columns": compared_heights,
		"biome_columns": compared_biomes,
		"chunks": coords.size(),
	}
	...
	for pair in [
		...
		for world_z in range(maxi(min_az, min_bz), mini(max_az, max_bz) + 1):
			for world_x in range(maxi(min_ax, min_bx), mini(max_ax, max_bx) + 1):
				if (
					biomes_a[_cache_index(a, world_x, world_z)]
					!= biomes_b[_cache_index(b, world_x, world_z)]
				):
...
func _validate_determinism(data) -> Dictionary:
	...
	for coord: Vector2i in coords:
		...
		for key: String in ["heights", "biomes", "stage7_water_types"]:
...
func _benchmark(runtime) -> Dictionary:
	...
	for sample: int in samples:
		total += sample
	var p95_index: int = mini(
		samples.size() - 1,
		ceili(float(samples.size()) * 0.95) - 1
	)
	return {
